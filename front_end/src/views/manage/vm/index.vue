<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="vmList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="合作商" align="center" show-overflow-tooltip>
        <template #default="scope">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="vmRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="供货时间" v-if="form.lastSupplyTime!==null && !isAdd">
          <span>{{ parseTime(form.lastSupplyTime, "{y}-{m}-{d} {h}:{i}:{s}") }}</span>
        </el-form-item>
        <el-form-item label="设备型号" v-if="!isAdd">
          {{ getVmTypeName(form.vmTypeId) }}
        </el-form-item>
        <el-form-item label="设备型号" prop="vmTypeId" v-if="isAdd">
        ...
        <el-form-item label="设备容量" v-if="!isAdd">
        ...
        <el-form-item label="合作商"  v-if="!isAdd">
          <template v-if="form.partnerId">
            {{ getPartnerName(form.partnerId) }}
          </template>
        </el-form-item>
        <el-form-item label="所属区域" prop="regionId" v-if="!isAdd">
          {{ getRegionName(form.regionId) }}
        </el-form-item>
        <el-form-item label="详细地址" prop="addr" v-if="!isAdd">
          <template v-if="form.partnerId">
            {{ form.addr }}
          </template>
        </el-form-item>
    ...
    <!-- 修改策略对话框 -->
    <el-dialog  v-model="policyOpen" width="500px" title="修改策略">
