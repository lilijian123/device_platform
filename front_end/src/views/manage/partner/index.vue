<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="partnerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分成比例"  align="center" width="120">
        <template #default="scope">
          {{ scope.row.divideRatio }}%
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary"  @click="resetPwd(scope.row.id)" v-hasPermi="['manage:partner:resetPwd']">重置密码</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="partnerRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="分成比例" prop="divideRatio">
          <el-input v-model="form.divideRatio" placeholder="请输入分成比例" />
        </el-form-item>
        <el-form-item label="创建时间" prop="createTime" v-if="form.id!=null">
          {{ form.createTime }}
        </el-form-item>
        <el-form-item label="登录账号" prop="account" v-if="form.id==null">
          <el-input v-model="form.account" placeholder="请输入登录账号" />
        </el-form-item>
        <el-form-item label="登录密码" prop="password" v-if="form.id==null">
    ...
    <!-- 查看合作商详情 -->
    <el-dialog title="合作商详情" v-model="partnerInfoOpen" width="550px" append-to-body>
      <el-descriptions column="2" border>
        <el-descriptions-item label="合作商名称">
          {{ form.partnerName || "无" }}
        </el-descriptions-item>
        <el-descriptions-item label="联系人">
          {{ form.contactPerson || "无" }}
        </el-descriptions-item>
        <el-descriptions-item label="联系电话">
          {{ form.contactPhone || "无" }}
        </el-descriptions-item>
        <el-descriptions-item label="分成比例">
          {{ form.divideRatio ? `${form.divideRatio}%` : "0%" }}
        </el-descriptions-item>
      </el-descriptions>
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="partnerInfoOpen = false">关 闭</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
