<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="regionList" @selection-change="handleSelectionChange">
      ...
      <!-- 新增点位数量列 -->
      <el-table-column label="点位数量" align="center" width="100">
        <template #default="scope">
          {{ scope.row.nodeCount ?? 0 }}
        </template>
      </el-table-column>
      <el-table-column label="备注信息" align="left" prop="remark" show-overflow-tooltip />
...
function getList() {
  ...
  listRegion(queryParams.value).then(response => {
    // 打印信息调试
    console.log("区域列表完整数据", response.rows);
