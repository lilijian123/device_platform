  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="合作商" prop="partnerId">
        <el-select v-model="queryParams.partnerId" placeholder="请选择合作商" clearable @change="handleQuery">
...
const data = reactive({
  ...
  rules: {
    ...
    businessType: [
      { required: true, message: "请选择商圈类型", trigger: "change" }
    ]
  }
});
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除广告点位管理编号为"' + _ids + '"的数据项？').then(function() {
    return delNode(_ids);
  }).then(() => {
    getList();
