...
public class TaskServiceImpl implements ITaskService {
    ...
    public int insertTaskDto(TaskDto taskDto) {
        ...
        if (taskDto.getProductTypeId().equals(DkdContants.TASK_TYPE_SUPPLY)) {
            ...
            if (CollUtil.isEmpty(details)) {
                throw new ServiceException("补货工单不能为空");
            }
            // 将dto转为po补充属性
    ...
    public int cancelTask(Task task) {
        ...
        {
            throw  new ServiceException("该工单已经取消");
        }
        //判断工单是否已经完成
        if(taskDb.getTaskStatus().equals(DkdContants.TASK_STATUS_FINISH)){
            throw new ServiceException("该工单已经完成");
        }
        //设置更新字段
     ...
     * @param productTypeId 工单的类型，决定任务的性质（投放、维修、补货、撤机）。
     */
    private void hasTask(String innerCode, Long productTypeId) {
        Task query = new Task();
        query.setInnerCode(innerCode);
        query.setProductTypeId(productTypeId);
        Long count = taskMapper.countSameTask(query);
        if(count > 0){
            throw new ServiceException("存在同类型的进行中或待办工单");
        }
    ...
    private void checkCreateTask(Long vmStatus, Long productTypeId) {
        ...
        // 如果是补货工单，且设备状态不是运营中，则抛出异常，因为设备不在运营状态无法进行补货
        ...
        // 如果是撤机工单，且设备状态不是运营中，则抛出异常，因为设备不在运营状态无法进行撤机
