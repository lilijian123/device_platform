...
import com.dkd.manage.service.impl.VendingMachineServiceImpl;
...
public class EmpController extends BaseController
{
    ...
    {
        List<Emp> empList = vendingMachineService.selectBusinessEmpByInnerCode(innerCode);
        if(empList.isEmpty()){
            return error("售货机不存在");
        }
    ...
    public AjaxResult operationList(@PathVariable("innerCode") String innerCode){
        List<Emp> empList = vendingMachineService.selectOperationEmpByInnerCode(innerCode);
        if(empList.isEmpty()){
            return error("售货机不存在");
        }
        return success(empList);
